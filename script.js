...
// function moveDataBetweenExcels() {
//   // Fetch data from "بيانات جديدة.xlsx"
//   fetch("asset/data/بيانات جديدة.xlsx")
//     .then((res) => res.arrayBuffer())
//     .then((newDataBuffer) => {
//       const newWorkbook = XLSX.read(newDataBuffer, { type: "array" });

//       // Fetch data from "شهادات اعدادي.xlsx"
//       return fetch("asset/data/شهادات اعدادي.xlsx")
//         .then((res) => res.arrayBuffer())
//         .then((templateDataBuffer) => {
//           const templateWorkbook = XLSX.read(templateDataBuffer, {
//             type: "array",
//           });

//           // Iterate through each tab in "بيانات جديدة.xlsx"
//           newWorkbook.SheetNames.forEach((newSheetName, index) => {
//             const newSheet = newWorkbook.Sheets[newSheetName];
//             const newData = XLSX.utils.sheet_to_json(newSheet, { header: 1 }); // Extract rows as array of arrays

//             // Find corresponding sheet in "شهادات اعدادي.xlsx"
//             const templateSheetName = templateWorkbook.SheetNames[index]; // Assuming same index order
//             if (!templateSheetName) {
//               console.error(`No corresponding sheet for index ${index}`);
//               return;
//             }
...
//             // Map rows from "بيانات جديدة.xlsx" to "شهادات اعدادي.xlsx"
//             newData.forEach((row, rowIndex) => {
//               if (rowIndex === 0) return; // Skip header row
//               row.forEach((cell, colIndex) => {
//                 // Update the corresponding cell in the template sheet
//                 const cellAddress = XLSX.utils.encode_cell({
//                   r: rowIndex,
//                   c: colIndex,
//                 });
//                 templateSheet[cellAddress] = { v: cell };
//               });
//             });
//           });

//           // Save the updated workbook
//           const updatedData = XLSX.write(templateWorkbook, {
//             bookType: "xlsx",
//             type: "array",
//           });
...
//           a.href = url;
//           a.download = "Updated_شهادات_اعدادي.xlsx";
//           a.click();
//           URL.revokeObjectURL(url);
//         });
//     })
//     .catch((err) => console.error("Error moving data:", err));
// }
